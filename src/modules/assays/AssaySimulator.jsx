...
const LINE_COLORS = ["#4ade80", "#60a5fa", "#f59e0b", "#f472b6", "#a78bfa", "#34d399"];
        ...
        <div className="assay-result">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={chartData} margin={{ left: 10, right: 10, bottom: 20 }}>
              ...
              <ReferenceLine x={result.gate_threshold.toFixed(2)} stroke="#f87171"
                strokeDasharray="5 4" label={{ value: "Gate", position: "top", fill: "#f87171", fontSize: 11 }} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      {result && view === "dose" && (
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={doseData} margin={{ left: 10, right: 10, bottom: 20 }}>
            ...
            <Tooltip />
            <Line type="monotone" dataKey="fl" stroke="#60a5fa" dot={false} strokeWidth={2} name="FL" />
          </LineChart>
        </ResponsiveContainer>
      )}
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={chartData} margin={{ left: 10, right: 10, bottom: 20 }}>
              ...
              <ReferenceLine y={result.threshold} stroke="#f87171" strokeDasharray="5 4"
                label={{ value: "Threshold", position: "right", fill: "#f87171", fontSize: 11 }} />
              ...
              return (
                <g key={band.name}>
                  <text x={lx + GEL_LANE_W / 2} y={GEL_PAD_TOP - 10}
                    textAnchor="middle" fontSize={10} fill="var(--fg-2, #aaa)"
                    style={{ maxWidth: GEL_LANE_W }}>
                    {band.name.length > 8 ? band.name.slice(0, 7) + "…" : band.name}
                  </text>
                  <rect x={lx} y={gelY(band.position) - 3}
                    width={GEL_LANE_W} height={6} fill="#4ade80" rx={2} opacity={0.9} />
                  <text x={lx + GEL_LANE_W / 2} y={gelY(band.position) + 16}
                    textAnchor="middle" fontSize={9} fill="#4ade80">{band.length} bp</text>
                </g>
              );
            })}
          </svg>
